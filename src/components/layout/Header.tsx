...
export function Header() {
  ...
  const handleExportCsv = () => {
    // Exporta os dados atuais filtrados em formato CSV
    const rows = [
      ["Instituto", "Data", "Abrangencia", "Turno", "Cenario", "Tipo", "Candidato", "Percentual", "Votos Validos", "Margem Erro", "Amostra"],
    ];
    ...
    filteredPolls.forEach((p) => {
      p.results.forEach((r) => {
        rows.push([
          ...
          `"${p.type}"`,
          `"${r.candidateName}"`,
          `${r.percentage.toString().replace(".", ",")}%`,
          `${(r.validPercentage ?? r.percentage).toString().replace(".", ",")}%`,
          `${p.marginOfError}`,
          `${p.sampleSize}`,
        ]);
      });
    ...
    link.setAttribute("download", `pesquisas_eleitorais_2026_compiladas_${new Date().toISOString().split("T")[0]}.csv`);
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between py-4 gap-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Compilador, Agregador Ponderado e Mapa Interativo de Pesquisas Eleitorais
              </p>
            </div>
          </div>
          ...
          <div className="hidden lg:flex items-center space-x-3 text-xs">
            <div className="bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700/60 flex items-center space-x-2">
              ...
              <span className="font-semibold text-white">{nationalData.institutes.length}</span>
            </div>

            <div className="bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700/60 flex items-center space-x-2">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
              <span className="text-slate-300">Pesquisas Compiladas:</span>
              <span className="font-semibold text-emerald-400">{allPolls.length}</span>
            </div>
            ...
            <div className="bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700/60 flex items-center space-x-2">
              ...
              <span className="font-medium text-slate-200">
                {nationalData.dateRange.start !== "-" ? nationalData.dateRange.start : "2026"} até {nationalData.dateRange.end !== "-" ? nationalData.dateRange.end : "Hoje"}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              title="Exportar dados compilados para CSV"
