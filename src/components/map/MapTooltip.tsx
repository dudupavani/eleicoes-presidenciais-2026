...
import { MapMetricKey, Poll, StateTseSummary } from "@/types/election";
...
import { FileCheck2, DollarSign, BadgeCheck } from "lucide-react";
...
export function MapTooltip({ summary, featuredPoll, metric, position }: MapTooltipProps) {
              ...
              <div className="flex items-center justify-between text-xs bg-slate-800/80 rounded-lg p-2 border border-slate-700/60">
                <span className="flex items-center space-x-1.5 text-slate-300">
                  <span
                    ...
                    style={{ backgroundColor: CANDIDATES[leader.candidateId]?.color || "#64748B" }}
                  />
                  <span className="truncate max-w-[140px]">{leader.candidateName}</span>
                </span>
                <span className="font-bold text-white">{formatPercent(leader.percentage)}</span>
              </div>
              <div className="text-[10px] text-slate-400 flex items-center gap-1">
                <BadgeCheck className="w-3 h-3 text-emerald-400" />
                <span>{featuredPoll.institute} • {featuredPoll.round} • {featuredPoll.date}</span>
              </div>
            </div>
          ) : (
            <div className="text-xs text-slate-400 py-1 mb-1">
              Sem pesquisa estadual real publicada para {summary.uf}.
            </div>
          )
        ) : (
          <div className="space-y-2">
